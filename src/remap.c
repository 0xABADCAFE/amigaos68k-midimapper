...
Channel* allocChannels(Table* iniTable)
{
	...
	Channel* c = (Channel*)AllocMem(MIDI_NUM_CHANNELS*sizeof(Channel),
...
{
	initDirectives();
	if (!(channels = allocChannels(tableList))) /* allocate the channels */
		return false;
...
/*
				printf("MSB : 0x%02X 0x%02X 0x%02X : ",
					(unsigned)(dBuf[i-3]),
					(unsigned)(dBuf[i-2]),
					(unsigned)(dBuf[i-1])
				);
...
/*
				printf("LSB : 0x%02X 0x%02X 0x%02X : ",
					(unsigned)(dBuf[i-3]),
					(unsigned)(dBuf[i-2]),
					(unsigned)(dBuf[i-1])
				);
